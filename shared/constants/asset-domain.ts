export interface AssetDomainRegistryEntry<
...
] as const satisfies readonly AssetDomainRegistryEntry<string>[]

export type AssetKindEntry =
  typeof ASSET_KIND_REGISTRY[number]
export type AssetKind = AssetKindEntry['token']

export const ASSET_APPROVAL_STATE_REGISTRY = [
  { token: 'draft', label: '초안', order: 10 },
  { token: 'approved', label: '승인', order: 20 },
  { token: 'archived', label: '보관', order: 30 },
] as const satisfies readonly AssetDomainRegistryEntry<string>[]

export type AssetApprovalStateEntry =
  typeof ASSET_APPROVAL_STATE_REGISTRY[number]
export type AssetApprovalState =
  AssetApprovalStateEntry['token']
...
const ASSET_APPROVAL_STATE_TOKEN_SET:
ReadonlySet<string> = new Set(
  ASSET_APPROVAL_STATE_REGISTRY.map(entry => entry.token),
)
...
export function isAssetApprovalState(
  value: unknown,
): value is AssetApprovalState {
  return (
    typeof value === 'string'
    && ASSET_APPROVAL_STATE_TOKEN_SET.has(value)
  )
}
