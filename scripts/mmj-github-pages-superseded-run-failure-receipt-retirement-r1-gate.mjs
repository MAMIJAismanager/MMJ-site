...
assert.equal(wrapper.includes("resolve(root, 'scripts/mmj-ui29-dispatch-input-verify.mjs')"), false, 'build admission authority is still reused by failure receipt')
...
assert.ok(wrapper.indexOf('mmj-ui29-failure-receipt-evidence-verify.mjs') < wrapper.indexOf('mmj-ui29-build-receipt.mjs'), 'CMS mutation can occur before failure evidence verification')
...
console.log('PASS_SEQUENTIAL_QUEUE_CONCURRENCY_AUTHORITY')
