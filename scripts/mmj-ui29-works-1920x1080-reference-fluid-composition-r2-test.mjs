...
assert.equal(reference.viewportLocked, false)
assert.equal(reference.lockEligible, true)
assert.equal(reference.physicalFitPhase, 'unmeasured')
...
assert.equal(scaledReference.viewportLocked, false)
assert.equal(scaledReference.lockEligible, true)
assert.equal(scaledReference.referenceScale, 0.8)
assert.equal(scaledReference.cardDensity, 'compact')
...
assert.equal(wide.viewportLocked, false)
assert.equal(wide.lockEligible, true)
...
assert.equal(mobile.lockEligible, false)
...
assert.equal(tablet.lockEligible, false)
...
assert.equal(desktopFlow.lockEligible, false)
...
assert.equal(threshold.viewportLocked, false)
assert.equal(threshold.lockEligible, true)
assert.equal(threshold.cardDensity, 'compact')
...
console.log('PASS_1920X1080_REFERENCE_CANDIDATE_PROFILE')
console.log('PASS_SCALED_REFERENCE_CANDIDATE_PROFILE')
...
console.log('PASS_PHYSICAL_LOCK_DEFERRED_FROM_PURE_LAYOUT_PROFILE')
